...
struct CXMLReader::SImplementation {
    ...
    static void EndElementHandler(void *userData, const XML_Char *name) {
        ...
        impl->DCurrentEntity.DType = SXMLEntity::EType::EndElement;
        impl->DCurrentEntity.DNameData = name;
        impl->DCurrentEntity.DAttributes.clear(); // Clear attributes for end elements
        impl->DHasEntity = true;
    }
    ...
    static void CharacterDataHandler(void *userData, const XML_Char *s, int len) {
        ...
        impl->DCurrentEntity.DType = SXMLEntity::EType::CharData;
        impl->DCurrentEntity.DNameData = std::string(s, len);
        impl->DCurrentEntity.DAttributes.clear(); // Clear attributes for character data
        impl->DHasEntity = true;
    }
    ...
    bool ReadEntity(SXMLEntity& entity, bool skipcdata) {
        ...
        while (!DSource->End()) {
            ...
            if (BytesRead > 0) {
                ...
                if (DHasEntity) {
                    if (skipcdata && DCurrentEntity.DType == SXMLEntity::EType::CharData) {
                        // Skip CDATA if skipcdata is true
                        DHasEntity = false;
                        continue;
                    }
                    entity = DCurrentEntity;
